...
acc=(Q'*rddot')'; %acceleration in the cartesian framse
...
%Initializes trajectory vectors
i=1; 
...
%%%%%%Device Dynamics
  ...
  MatOut= matOut(m,l,th, th_dot);
    ...
    %Dynamics in Operational=admissible space
    H=R_inv'*G'*M*G*R_inv\eye(2);
    Mstar= R_inv'*G'*M*G*R_inv; 
    Fa_in_v=(R_inv'*G'*M*G*Rdot_inv+R_inv'*G'*M*Gdot*R_inv)*rdot0';
...
%      H1= Q'*R_inv'*G'*M*G*R_inv*Q\eye(2);
     EMM1=Q'*R_inv'*G'*M*G*R_inv*Q; %%%For plotting purposes  
...
%% Trajectory
    ...
    rdot(i+1,1)=rddot(1)*dt+rdot0(1); 
    r(i+1,1)=rddot(1)/2*dt^2+rdot0(1)*dt+r0(1);
  
        %in admissable space coordinates
%      acc1= H*(-1)*(Fa_in_v*rdot0'+Fa_c);
     rddot(2)=(-Fa_c(2)-Fa_in_v(2)-Mstar(1,2)*rddot(1))/Mstar(2,2);
     rdot(i+1,2)=rddot(2)*dt+rdot0(2); 
     r(i+1,2)=rddot(2)/2*dt^2+rdot0(2)*dt+r0(2); 
